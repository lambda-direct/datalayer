...
export default class SelectTRBWithFiveJoins<COLUMN extends
ColumnType, T1, T2, T3, T4, T5, MODEL> extends AbstractJoined<MODEL> {
  private _join1: Join<COLUMN, T1>;
  private _join2: Join<COLUMN, T2>;
  private _join3: Join<COLUMN, T3>;
  private _join4: Join<COLUMN, T4>;
  private _join5: Join<COLUMN, T5>;
  ...
  public constructor(tableName: string, session: Session,
    ...
    join4: Join<COLUMN, T4>, join5: Join<COLUMN, T5>,
  ...
  public execute = async (): Promise<SelectResponseFiveJoins<MODEL, T1, T2, T3, T4, T5>> => {
    const queryBuilder = Select
      ...
      .joined([this._join1, this._join2, this._join3, this._join4, this._join5])
    ...
    if (result.isLeft()) {
      ...
      const objectsFive = QueryResponseMapper.map(parentFive, result.value);
